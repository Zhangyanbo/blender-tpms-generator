...
This is *not* a marching-cubes / iso-surface mesh of the familiar
`sin x cos y + sin y cos z + sin z cos x = 0` approximation. The surfaces
are evaluated from their exact Weierstrass representations: every vertex
lies on the true minimal surface (mean curvature exactly zero), the quads
follow the natural parameter lines, and the mesh is exactly periodic, so
tiled cells weld seamlessly.
...
*whole* fundamental patches, not a box-clipped chunk. Schwarz D patches
...
这**不是**对常见等值面近似 `sin x cos y + sin y cos z + sin z cos x = 0`
做 marching cubes 得到的网格。三种曲面都由精确的 Weierstrass 表示直接求
值：每个顶点都严格落在真正的极小曲面上（平均曲率恒为零），四边形沿曲面
天然参数线分布，网格精确周期，平铺的晶胞无缝焊合。
...
- **平滑着色** —— 使用精确解析法向（Weierstrass 数据的 Gauss 映射），而
  非平均面法向。

关于块体边缘的说明：晶胞是由**完整**基本片构成的一个精确平移单元，不是
按立方体裁剪的碎块。Schwarz D 的基本片恰好终止在晶胞面上，Gyroid 的外伸
...
- 积分采用逐段推进的 16 点 **Gauss–Legendre** 复合求积。靠近分支点的线段
  ...
  异角点处不损失精度。
- 把基本片拼成一个立方晶胞的等距变换是**数值推导的，而非照抄论文**：把
